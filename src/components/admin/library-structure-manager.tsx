...
export function LibraryStructureManager({
  ...
  const [chapterForm, setChapterForm] = useState({ number: 1, title: "" });
  ...
  const totalArticles = useMemo(
    () => chapters.reduce((s, c) => s + c.articles.length, 0),
    [chapters],
  );
  ...
  async function handleAddChapter() {
    ...
    startTransition(async () => {
      ...
      fd.append("title", chapterForm.title);
      fd.append("number", String(chapterForm.number));
      const result = await createLibraryChapter(lawId, fd);
      if (result?.error) {
        addToast({ message: result.error, variant: "error" });
      } else {
        addToast({ message: "فصل اضافه شد.", variant: "success" });
        setChapterForm({ number: chapterForm.number + 1, title: "" });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <SummaryStat icon={Layers} label="باب" value={toPersian(chapters.length)} color="from-violet-500/10 to-violet-500/5" textColor="text-violet-600" />
      ...
      <div className="rounded-3xl border border-border bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-base font-extrabold text-primary-navy">فصل‌ها</h3>
            <p className="mt-0.5 text-xs text-text-muted">هر قانون می‌تواند شامل چند فصل باشد</p>
          </div>
          <Button
            variant="accent"
            size="sm"
          ...
          >
            ...
            <span className="mr-1">افزودن فصل</span>
          </Button>
        </div>
        {addingChapter && (
          <div className="mt-3 grid grid-cols-1 gap-2 rounded-2xl border border-border bg-surface-background p-3 sm:grid-cols-[6rem_1fr_auto]">
            ...
            />
            <Button onClick={handleAddChapter} disabled={pending}>
              ثبت
            </Button>
          </div>
        )}
      ...
      <div className="space-y-3">
        {chapters.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-border bg-white p-12 text-center">
            ...
            <h3 className="mt-4 text-base font-bold text-primary-navy">هنوز فصلی ثبت نشده</h3>
            <p className="mt-1 text-sm text-text-muted">با افزودن اولین فصل، ساختار قانون را شروع کنید.</p>
          </div>
        ) : (
          chapters.map((chapter) => {
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2 border-b border-border bg-gradient-to-l from-violet-500/5 to-transparent p-4">
                  <button
                    type="button"
                    onClick={() =>
                      setExpandedChapters((p) => ({ ...p, [chapter.id]: !p[chapter.id] }))
                    }
                    className="flex min-w-0 flex-1 items-center gap-3"
                  >
                    <div className="inline-flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-violet-500 to-violet-600 text-sm font-black text-white shadow-md shadow-violet-500/30">
                      {toPersian(chapter.number)}
                    </div>
                    <div className="min-w-0 text-start">
                      <div className="text-sm font-extrabold text-primary-navy line-clamp-1">
                        باب {toOrdinalWord(chapter.number)}: {chapter.title}
                      </div>
                ...
                {expandedChapters[chapter.id] && (
                  <div className="space-y-2 p-4">
                    {chapter.articles.length === 0 ? (
                      <p className="rounded-xl border border-dashed border-border bg-surface-background p-4 text-center text-xs text-text-muted">
